...
shell (`render`, `input`, `storage`, `pause`, `app`) is thin and mostly untested —
the exception is any small pure helper it factors out, such as `src/pause/`, which
carries its own unit tests. `app` runs the sim at a fixed 120 Hz via an
accumulator, decoupled from `requestAnimationFrame` rendering, which interpolates
