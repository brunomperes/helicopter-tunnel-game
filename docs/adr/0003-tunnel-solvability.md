...
from scroll speed, thrust accel, gravity, and terminal velocity). Obstacles never
occupy consecutive slices. The result: every tunnel the generator can produce is
completable by a perfect player, so every crash is player error, never an RNG
death.
